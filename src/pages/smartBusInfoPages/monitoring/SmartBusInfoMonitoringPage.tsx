...
import { SMART_BUS_INFO_MAP_CONFIG } from "../../../constants/smartBusInfo.constant";
...
/**
 * 스마트 버스정보 모니터링 page 컴포넌트
 * @returns {JSX.Element}
 */
export const SmartBusInfoMonitoringPage = (): JSX.Element => {
    /** 인증 처리용 키클록 토큰 정보 */
    ...
    useEffect(() => {
        const fetchBaseMapData = async () => {
            if (keyCloakToken) {
                const responses = await fetchFeatureAndStyleData(SMART_BUS_INFO_MAP_CONFIG, keyCloakToken);
                if (responses) {
    ...
    return (
        <MonitoringPageContainer>
            <BaseMap
                mapCreationConfig={{ ...SMART_BUS_INFO_MAP_CONFIG, kakaoMapUrlConfig: kakaoMapUrlConfig }}
                layerCreationConfig={layerCreationConfig}
                /* selectedFeatureCallback={selectedFeatureCallback} */
            />
            <TodoCurrentPageText>🚌 스마트 버스정보 모니터링 page 컴포넌트 🖥️</TodoCurrentPageText>
        </MonitoringPageContainer>
    );
};
